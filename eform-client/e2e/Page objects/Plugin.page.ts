...
class PluginRowObject {
  ...
  public async getRow(rowNum: number): Promise<PluginRowObject> {
    ...
    this.actionMenuBtn = await $('#action-items-'+this.rowNumber).$('#actionMenu');
  ...
  async enableOrDisablePlugin(timeout = 100000) {
    ...
    await this.actionMenuBtn.waitForDisplayed({ timeout: 40000 });
    await this.actionMenuBtn.waitForClickable({ timeout: 40000 });
    await this.actionMenuBtn.click();
    await browser.pause(500);
    
    // Then click on the status button inside the menu
    await this.statusBtn.waitForDisplayed({ timeout: 40000 });
    await this.statusBtn.waitForClickable({ timeout: 40000 });
