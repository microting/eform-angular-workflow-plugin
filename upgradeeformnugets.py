...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir(os.path.join("eFormAPI", "Plugins", "Workflow.Pn", "Workflow.Pn"))
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.eFormWorkflowBase',
            'Microting.EformAngularFrontendBase', 'Sentry'
        ]
        process_repository('Workflow.Pn.csproj', packages, 'eform-angular-workflow-plugin')

        os.chdir("..")
        os.chdir("Workflow.Pn.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NSubstitute', 'NUnit', 'NUnit3TestAdapter',
            'NUnit.Analyzers', 'Testcontainers', 'Testcontainers.Mariadb'
        ]
        process_repository('Workflow.Pn.Test.csproj', packages, 'eform-angular-workflow-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            run_command(f"github_changelog_generator -u microting -p eform-angular-workflow-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
